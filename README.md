...
- **Exact (image)** — default. Rasters the whole frame to a 2× PNG wrapped in a
  single-image email. Fonts, background, and layout reproduce **pixel-for-pixel**
  (the reliable path when the design uses custom fonts, overlap, or effects that
  email HTML cannot express). Trade-off: the text is not selectable.
- **Text (editable)** — semantic mapping (text stays real text, icons/images
  rasterized). Easy to edit/translate; approximates the layout within email-HTML
  limits.
...
- The plugin window is resizable via the bottom-right drag handle.
